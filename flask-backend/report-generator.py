...
# def generate_report_old(name, general, symptoms, exposure, ai_comments, risk_level_summary):
#     date_completed = datetime.now().strftime("%Y-%m-%d")
#     context = {'name': name, 'date_completed': date_completed, 'general': general, 'symptoms': symptoms, 'exposure': exposure, 'ai_comments': ai_comments, 'risk_level_summary': risk_level_summary}
...
#     config = pdfkit.configuration(wkhtmltopdf='/usr/local/bin/wkhtmltopdf')
#     output_pdf = 'doctor_report_old.pdf'
#     pdfkit.from_string(output_text, output_pdf, configuration=config, options={"enable-local-file-access": ""})
...
def generate_report(name, age, gender, location, summary, technical):
    ...
    html_template = 'TuberculosisAIPreScreener.html'
    template = template_env.get_template(html_template)
    output_text = template.render(context)

    config = pdfkit.configuration(wkhtmltopdf='/usr/local/bin/wkhtmltopdf')
    output_pdf = 'doctor_report_new.pdf'
    # pdfkit.from_string(output_text, output_pdf, configuration=config, options={"enable-local-file-access": ""})
    pdfkit.from_string(output_text, output_pdf, configuration=config, options={"enable-local-file-access": True})
